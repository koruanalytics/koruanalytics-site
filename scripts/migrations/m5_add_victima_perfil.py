"""
scripts/migrations/m5_add_victima_perfil.py

Migration M5: Add victima_perfil column to silver_news_enriched and gold_incidents.

This migration adds the victima_perfil field for victim targeting analysis.
Safe to run multiple times (idempotent).

Valid values: comerciante, transportista, autoridad, candidato, periodista,
              abogado, empresario, estudiante, mujer, menor, civil, 
              desconocido, no_aplica

Usage:
    python -m scripts.migrations.m5_add_victima_perfil --check
    python -m scripts.migrations.m5_add_victima_perfil --apply
    python -m scripts.migrations.m5_add_victima_perfil --rollback
...
MIGRATION_NAME = "M5_victima_perfil"
...
def check_migration_status(db_path: str = DB_PATH) -> dict:
    ...
    silver_exists = check_column_exists(con, "silver_news_enriched", "victima_perfil")
    ...
    gold_exists = check_column_exists(con, "gold_incidents", "victima_perfil")
...
def apply_migration(db_path: str = DB_PATH, dry_run: bool = False) -> dict:
    """
    Apply the M5 migration to add victima_perfil column.
    ...
    if not check_column_exists(con, "silver_news_enriched", "victima_perfil"):
        sql = """
            ALTER TABLE silver_news_enriched 
            ADD COLUMN victima_perfil VARCHAR
        """
        if dry_run:
            logger.info(f"[DRY RUN] Would execute: {sql}")
            results["changes"].append({
                "table": "silver_news_enriched",
                "action": "ADD COLUMN victima_perfil",
                "status": "pending"
            })
        else:
            try:
                con.execute(sql)
                logger.success("Added victima_perfil to silver_news_enriched")
                results["changes"].append({
                    "table": "silver_news_enriched",
                    "action": "ADD COLUMN victima_perfil",
                    "status": "success"
        ...
        logger.info("silver_news_enriched.victima_perfil already exists")
        results["changes"].append({
            "table": "silver_news_enriched",
            "action": "ADD COLUMN victima_perfil",
    ...
    if not check_column_exists(con, "gold_incidents", "victima_perfil"):
        sql = """
        ...
        if dry_run:
            ...
            try:
                con.execute(sql)
                logger.success("Added victima_perfil to gold_incidents")
                results["changes"].append({
                    "table": "gold_incidents",
                    "action": "ADD COLUMN victima_perfil",
                    "status": "success"
        ...
        logger.info("gold_incidents.victima_perfil already exists")
        results["changes"].append({
            "table": "gold_incidents",
            "action": "ADD COLUMN victima_perfil",
            "status": "already_exists"
...
def rollback_migration(db_path: str = DB_PATH, dry_run: bool = False) -> dict:
    """
    Rollback the M5 migration by removing victima_perfil column.
    WARNING: This will delete all victima_perfil data!
    ...
    if check_column_exists(con, "silver_news_enriched", "victima_perfil"):
        sql = "ALTER TABLE silver_news_enriched DROP COLUMN victima_perfil"
        if dry_run:
            logger.info(f"[DRY RUN] Would execute: {sql}")
            results["changes"].append({
                "table": "silver_news_enriched",
                "action": "DROP COLUMN victima_perfil",
                "status": "pending"
            })
        else:
            try:
                con.execute(sql)
                logger.warning("Dropped victima_perfil from silver_news_enriched")
                results["changes"].append({
                    "table": "silver_news_enriched",
                    "action": "DROP COLUMN victima_perfil",
                    "status": "success"
        ...
        logger.info("silver_news_enriched.victima_perfil does not exist")
        results["changes"].append({
            "table": "silver_news_enriched",
            "action": "DROP COLUMN victima_perfil",
            "status": "not_exists"
    ...
    if check_column_exists(con, "gold_incidents", "victima_perfil"):
        sql = "ALTER TABLE gold_incidents DROP COLUMN victima_perfil"
        if dry_run:
            logger.info(f"[DRY RUN] Would execute: {sql}")
            results["changes"].append({
                "table": "gold_incidents",
            ...
            try:
                con.execute(sql)
                logger.warning("Dropped victima_perfil from gold_incidents")
                results["changes"].append({
                    "table": "gold_incidents",
                    "action": "DROP COLUMN victima_perfil",
                    "status": "success"
        ...
        logger.info("gold_incidents.victima_perfil does not exist")
        results["changes"].append({
            "table": "gold_incidents",
            "action": "DROP COLUMN victima_perfil",
            "status": "not_exists"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="M5 Migration: Add victima_perfil column"
    )
    ...
    if args.check:
        ...
        print("=" * 50)
        print("\n⚠️  WARNING: This will DELETE all victima_perfil data!")
